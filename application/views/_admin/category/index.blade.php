...
<div class="row">
  ...
  <div class='col-md-12 col-lg-12 col-sm-12 text-right mb-3'>
    <a class='btn btn-primary' href='<?php echo site_url('admin/category/create'); ?>'> Nova Categoria </a>
  </div>

  <div class="col-lg-12 col-sm-12 col-md-12">
		<form role="form" id='searchCategory' method='GET' action="{{ base_url('admin/category/index') }}">
			<div class="card">
          ...
          Ferramentas de Pesquisa
					...
					<div class='row'>
						...
						<div class="form-group col-md-4">
							...
							<select name='categoryStatus' id='categoryStatus' class="form-control">
								...
								<option value='1' @if(isset($search_data['category_status']) && $search_data['category_status'] == 1) selected @endif>Ativo</option>
								<option value='0' @if(isset($search_data['category_status']) && $search_data['category_status'] === 0) selected @endif>Inativo</option>
							</select>
						</div>
                ...
                <option value='category_status' @if(isset ($search_data['categoryOrderBy']) && $search_data['categoryOrderBy'] === 'category_status') selected @endif>Status</option>
              </select>
						</div>
              ...
              <div class="card-body">
                <div class='row'>
                  ...
                  <div class='col-md-6 col-lg-6 col-sm-12'>
                    ...
                    <h5 class="card-title @if($dado_tabela['category_status'] == 1) badge badge-success @else badge badge-danger @endif">@if($dado_tabela['category_status'] ==1) Ativo @else Inativo @endif  </h5>
                  </div>

                  <div class='col-md-12 col-lg-12 col-sm-12'>
                    <p class="card-text"><small class="text-muted">Descrição:</small></p>
                    <p class="card-text">{!! $dado_tabela['category_description'] !!}</p>
                  </div>
                </div>
              </div>
              <div class="card-footer text-right">
                <div class='btn-group'>
                  <a class='btn btn-primary' href="{{ base_url('admin/category/edit/'.$dado_tabela['id_category']) }}"> Editar </a>
                  <button class="btn @if($dado_tabela['category_status'] == 0) btn-success @else btn-danger @endif" data-id="{{ $dado_tabela['id_category'] }}" id="changeStatus"> @if($dado_tabela['category_status'] == 0) Ativar @else Inativar @endif </button>
                </div>
              </div>
            </div>
          </div>
...
<script>
  $('#changeStatus').on('click', function() {
    ...
    Swal.fire({
      title: "Atenção!",
      text: "Você está prestes a mudar o status dessa categoria, você tem certeza que deseja continuar?",
      icon: "warning",
      ...
      if (result.value) {
        var request = $.ajax({
          url: "{{ base_url('admin/category/changeStatus/') }}"+id,
          method: "POST",
          data: { id : id },
          dataType: "json"
        });
