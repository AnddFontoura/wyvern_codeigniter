...
<div class="row">
  ...
  <div class='col-md-12 col-lg-12 col-sm-12 text-right mb-3'>
    <a class='btn btn-primary' href="{{ site_url('admin/permissioncategory') }}"> Listar Categorias de Permissões </a>
  </div>
  ...
  <div class="col-lg-12 col-sm-12 col-md-12">
        ...
        <div class='row'>
  				...
  				<div class="form-group col-md-12 col-lg-12 col-sm-12 mb-3">
  					<label for="permissionCategoryName">Nome da Permissão: <i class='fa fa-question-circle' data-toggle="tooltip" data-placement="top" title="Nome que estará na pesquisa ou exibição"></i> </label>
  					<input type="text" value='@if(isset($edit_data)) {{ $edit_data[0]['permission_name'] }} @endif' class="form-control" id="permissionCategoryName"  name="permissionCategoryName" placeholder="Nome da Permissão; será exibido nos campos de busca e menu" maxlength='254'>
  				</div>
  				...
  				<div class="form-group col-md-12 col-lg-12 col-sm-12 mb-3">
  					<label for="permission_status">Incluir Permissões CRUD?? <i class='fa fa-question-circle' data-toggle="tooltip" data-placement="top" title="Categoria Inativa não será exibida em pesquisa em sua loja/site"></i> </label>
  					<select name='permissionMakeCrud' id='permissionMakeCrud' class="form-control">
...
<script>
	...
	$('#cadastrarPermissao').on('click', function() {
		...
		data.append ( "permissionCategoryStatus" , $( "select#permissionCategoryStatus option:checked" ).val());
  	data.append ( "permissionCategoryName" , $('#permissionCategoryName').val());
		...
		var request = $.ajax({
			url: "@if(isset($edit_data)) {{ base_url('admin/permissioncategory/save/'.$edit_data[0]['id_permission']) }} @else {{ base_url('admin/permissioncategory/save') }}  @endif",
			method: "POST",
			data: data,
			dataType: "json",
